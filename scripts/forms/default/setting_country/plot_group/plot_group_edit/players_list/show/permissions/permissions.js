...
import { plotGroupPlayerSelectedShowDefaultForm } from "../plot_group_player_selected_show";
 ...
 */
export function plotGroupPlayerPermissionsEditDefaultForm(player, plotGroupId, targetData, isPlotAdmin = false) {
    ...
    let target = plotGroupData.players.find(d => d.id == targetData?.id) || { id: targetData?.id, permissions: [] };
    ...
    form.show(player).then(rs => {
        ...
        const index = plotGroupData?.players.findIndex(d => d.id == target.id);
        if (index != -1) {
