...
world.afterEvents.playerSpawn.subscribe((ev) => {
    ...
    lastPlayers = lastPlayers.filter((p: any) => p.id != player.id);
...
world.afterEvents.playerLeave.subscribe((ev) => {
    const { playerId } = ev;
    let lastPlayers = JSON.parse(playTimeDB.get('worldLoginPlayers') || '[]');
    const data = lastPlayers.find((p: any) => p.id == playerId);
    if (!data) return;
    addPlayTimeRange(playerId, data.loginTime, Date.now());
    lastPlayers = lastPlayers.filter((p: any) => p.id != playerId);
    playTimeDB.set('worldLoginPlayers', JSON.stringify(lastPlayers));
});

function addPlayTimeRange(playerId: any, startMs: any, endMs: any) {
...
function splitByDay(startMs: any, endMs: any) {
    const result = [];
...
function pruneDailyMap(daily: any, keepDays = 30) {
...
function getMonthKey(dateKey: any) {
    return dateKey.slice(0, 7);
}

export function showPlayTimeStats(player: any) {
    const id = player.id;
...
function getCurrentSessionMinutes(playerId: any) {
    const list = JSON.parse(
        playTimeDB.get('worldLoginPlayers') || '[]'
    );
    const data = list.find((p: any) => p.id === playerId);
    if (!data) return 0;

    return Math.floor((Date.now() - data.loginTime) / 1000 / 60);
}

function getCurrentSessionSegments(playerId: any) {
    const list = JSON.parse(
        playTimeDB.get('worldLoginPlayers') || '[]'
    );
    const data = list.find((p: any) => p.id === playerId);
...
export function hasJoinedInPeriodWithinDailyRetention(playerId: any, startDate: any, endDate: any) {
    const daily = JSON.parse(
        playTimeDB.get(`player_${playerId}_daily`) || '{}'
    );
