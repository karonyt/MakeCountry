...
import { plotGroupEditRolesListDefaultForm } from "../roles_list.js";
 ...
 */
export function roleAddPlotGroupDefaultForm(player: any, plotGroupId: any, isPlotAdmin: any, search = false, keyword = ``) {
    ...
    for (const id of roleIds) {
        ...
        if (plotGroupData.roles.find((d: any) => d?.id == roleData.id)) continue;
    ...
    form.show(player).then(rs => {
        ...
        switch (rs.selection) {
            ...
            default: {
                ...
                plotGroupData.roles = plotGroupData.roles.filter((r: any) => r.id != target.id);
