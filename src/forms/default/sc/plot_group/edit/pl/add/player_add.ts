...
import { plotGroupEditPlayersListDefaultForm } from "../players_list.js";
...
export function playerAddPlotGroupDefaultForm(
    ...
    for (const p of players) {
        ...
        if (plotGroupData.players.some((d: any) => d.id === p.id)) continue;
 ...
 * 検索フォーム
 */
function openSearchForm(player: any, plotGroupId: any, isPlotAdmin: any) {
    ...
    form.show(player).then(rs => {
        ...
        // @ts-ignore TS(2345): Argument of type 'string | number | boolean | unde... Remove this comment to see the full error message
