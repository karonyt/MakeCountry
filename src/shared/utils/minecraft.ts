...
import { checkOnlyRole } from "@/domain/country/permissions.js";
import { areCountriesInSameFederation } from "@/domain/country/relationships/federation.js";

const dynamicPropertiesByName: Record<string, DynamicProperties> = Object.create(null);

function getDynamicProperties(name: string) {
    return dynamicPropertiesByName[name] ??= new DynamicProperties(name);
}
 ...
 * @returns {string}
 */
export function GetChunkPropertyId(rawX: any, rawZ: any, dimension = `overworld`) {
 ...
 * @returns {string}
 */
export function GetPlayerChunkPropertyId(player: any) {
...
export function GetAndParsePropertyData(id: string, database?: DynamicProperties): any | undefined {
    if (!database) {
        if (id.includes('_')) {
            const database = getDynamicProperties(id.split('_')[0]);
    ...
    // @ts-ignore TS(2339): Property 'get' does not exist on type 'never'.
 ...
 * @returns 
 */
export function StringifyAndSavePropertyData(id: any, data: any, database = undefined) {
    if (!database) {
        if (id.includes('_')) {
            const database = getDynamicProperties(id.split('_')[0]);
    ...
    // @ts-ignore TS(2339): Property 'set' does not exist on type 'never'.
    database.set(id, JSON.stringify(data));
    return;
};
 ...
 */
export function ConvertChunk(rawX: any, rawZ: any) {
    const x = Math.floor(rawX / 16);
    const z = Math.floor(rawZ / 16);
    return { x, z }
};
 ...
 */
export function CheckPermission(player: any, permission: any) {
    // ====== playerId 解決 ======
    let playerId;
    ...
    // ====== 共通権限判定 ======
    const hasRolePermission = (playerData: any, perm: any) => {
    ...
    if (plot?.enable && plot.type === "private") {
        ...
        const playerEntry = plot.players?.find((p: any) => p.id === playerData?.id);
    ...
    if (plot?.enable && plot.type === "embassy" && plot.owner) {
        ...
        if (plot.countries?.some((c: any) => c.id === playerData?.country && c.permissions.includes(permission))) {
            return false;
        }
        return true;
    }
    ...
    if (plot?.enable && plot.type === "public") {
        if (plot.players?.some((p: any) => p.id === playerData?.id && p.permissions.includes(permission))) {
            return false;
        }

        if (playerData?.country === chunkData.countryId) {
            if (plot.permissions?.includes(permission)) return false;
            if (plot.roles?.some((r: any) => r.permissions.includes(permission) && playerData.roles.includes(r.id))) {
                return false;
            }
            const countryData = getCountry(chunkData.countryId);
            if (countryData?.owner === playerData?.id) return false;
    ...
    if (chunkData.countryId) {
        ...
        const playerCountryData = getCountry(playerData?.country);
        if (areCountriesInSameFederation(countryData, playerCountryData)) {
            return !countryData.federationPermission?.includes(permission);
        }
 ...
 * @returns {boolean} true = キャンセル / false = 許可
 */
export function CheckPermissionFromLocation(player: any, x: any, z: any, dimensionId: any, permission: any) {
    // ===== AdminMode =====
    ...
    if (chunkData.plot?.group) {
        const plotGroupRaw = getDynamicProperties('plotgroup').get(`plotgroup_${chunkData.plot.group}`);
        plot = plotGroupRaw ? JSON.parse(plotGroupRaw) : chunkData.plot;
    }
    ...
    if (plot?.enable && plot.type === "public") {
        ...
        if (playerData.country === chunkData.countryId) {
            if (plot.permissions?.includes(permission)) return false;
            if (plot.roles?.some((r: any) => r.permissions.includes(permission) &&
 ...
 * @returns {boolean}
 */
export function HasPermission(player: any, permission: any) {
...
export function getRandomInteger(min: any, max: any) {
 ...
 * @returns 
 */
export function isDecimalNumber(value: any) {
 ...
 * @returns {boolean}
 */
export function isDecimalNumberZeroOK(value: any) {
    if (value == 0) {
        return true;
    };
    const integerRegex = /^[1-9]\d*$/;
...
export function getTimeBefore(time: any, minutesBefore: any) {
    // 与えられた時間と分から、合計分を計算
 ...
 * @returns {string}
 */
export function playerNameToId(playerName: any) {
    const playerDataBase = new DynamicProperties('player');
    const playerIds = playerDataBase.idList;
    return playerIds.find((id: any) => playerName === GetAndParsePropertyData(id).name).split('_')[1];
};
 ...
 * @returns {string}
 */
export function langChangeItemName(itemName: any) {
