 ...
 * @param {boolean} peace 
 */
export function MakeCountry(owner: any, reason: any, name = `country`, invite = true, peace = config.defaultPeace) {
    ...
    const [ownerRole, adminRole, peopleRole] = CreateRole([
        // @ts-ignore TS(2322): Type 'string' is not assignable to type 'never'.
    ...
    const countryData = {
        ...
        //連邦国の権限
        federationPermission: [`blockUse`, `entityUse`, `openContainer`, `itemUse`, `noTarget`, `setHome`, `publicHomeUse`],
        //敵対国の権限
        hostilityPermission: [],
        //友好国の権限
...
export function GenerateChunkData(x: any, z: any, dimensionId: any, ownerId = undefined, countryId = undefined, price = config.defaultChunkPrice, special = false) {
    const chunkData = {
        x: x,
        z: z,
 ...
 * @returns {number}
 */
export function calculationCountryPower(countryId: any) {
    const countryData = GetAndParsePropertyData(countryId);
    let countryPower = 0;
    const federationMembers = getFederationMemberIds(countryData).filter((id: any) => Number(id) !== Number(countryData.id));
    countryPower = countryData.money + countryData.members.length * 20 + countryData.territories.length * 10 + countryData.resourcePoint + countryData.alliance.length * 5 + federationMembers.length * 8 - countryData.hostility.length * 15;
    return countryPower;
};
...
export function DeleteCountry(countryId: any) {
    ...
    cleanupRelationsDelete(countryDB, cid, [
        ...
        "mergeRequestSend",
        "mergeRequestReceive",
        "federationRequestSend",
        "federationRequestReceive",
        "friendlyRequestSend",
        "friendlyRequestReceive",
        "applicationPeaceRequestSend",
        "applicationPeaceRequestReceive"
 ...
 * @param {string} color
 */
export function CreateRoleToCountry(countryId: any, name: any, permissions = [], iconTextureId = `stone`, color = `e`) {
 ...
 * @returns 
 */
export function DeleteRole(player: any, roleId: any, countryId: any, deleteCountry = false) {
    ...
    roleData.members.forEach((memberId: any) => {
        try {
            const memberData = GetAndParsePropertyData(`player_${memberId}`);
            memberData.roles = memberData.roles.filter((r: any) => r != roleId);
            StringifyAndSavePropertyData(`player_${memberId}`, memberData);
        } catch (error) {
            console.warn(error);
        };
    });
    countryData.roles = countryData.roles.filter((r: any) => r != roleId);
 ...
 * @param {string} countryId 
 */
export function AddHostility(mainCountryId: any, countryId: any) {
 ...
 * @param {string} countryId 
 */
export function RemoveHostility(mainCountryId: any, countryId: any) {
    const CountryData = GetAndParsePropertyData(`country_${countryId}`);
    const MainCountryData = GetAndParsePropertyData(`country_${mainCountryId}`);
    try {
        CountryData.hostility = CountryData.hostility.filter((id: any) => id != Number(mainCountryId));
        MainCountryData.hostility = MainCountryData.hostility.filter((id: any) => id != Number(countryId));
        StringifyAndSavePropertyData(`country_${countryId}`, CountryData);
        StringifyAndSavePropertyData(`country_${mainCountryId}`, MainCountryData);
    } catch (error) {
        console.warn(error);
 ...
 * @param {string} countryId 
 */
export function RemoveFriendly(mainCountryId: any, countryId: any) {
    const CountryData = GetAndParsePropertyData(`country_${countryId}`);
    const MainCountryData = GetAndParsePropertyData(`country_${mainCountryId}`);
    try {
        CountryData.friendly = CountryData.friendly.filter((id: any) => id != Number(mainCountryId));
        MainCountryData.friendly = MainCountryData.friendly.filter((id: any) => id != Number(countryId));
        StringifyAndSavePropertyData(`country_${countryId}`, CountryData);
        StringifyAndSavePropertyData(`country_${mainCountryId}`, MainCountryData);
    } catch (error) {
        console.warn(error);
 ...
 * @param {string} countryId 
 */
export function RemoveAlliance(mainCountryId: any, countryId: any) {
    const CountryData = GetAndParsePropertyData(`country_${countryId}`);
    const MainCountryData = GetAndParsePropertyData(`country_${mainCountryId}`);
    try {
        CountryData.alliance = CountryData.alliance.filter((id: any) => id != Number(mainCountryId));
        MainCountryData.alliance = MainCountryData.alliance.filter((id: any) => id != Number(countryId));
        StringifyAndSavePropertyData(`country_${countryId}`, CountryData);
        StringifyAndSavePropertyData(`country_${mainCountryId}`, MainCountryData);
    } catch (error) {
        console.warn(error);
 ...
 * @param {string} name 
 */
export function MakeInternationalOrganization(owner: any, ownerCountryId: any, name: any) {
 ...
 * @param {Number} countryId 
 */
export function playerCountryJoin(player: any, countryId: any) {
    try {
        const countryData = GetAndParsePropertyData(`country_${countryId}`);
 ...
 * @param {Player} player 
 */
export function playerCountryLeave(player: any) {
    try {
        const playerData = GetAndParsePropertyData(`player_${player.id}`);
        const countryId = playerData.country;
        const countryData = GetAndParsePropertyData(`country_${countryId}`);
        if (countryData?.members) {
            countryData.members = countryData.members.filter((m: any) => m != playerData.id);
            StringifyAndSavePropertyData(`country_${countryId}`, countryData);
        };
        playerData.roles = [];
...
export function playerCountryKick(player: any) {
    try {
        const playerData = GetAndParsePropertyData(`player_${player.id}`);
        const countryId = playerData.country;
        const countryData = GetAndParsePropertyData(`country_${countryId}`);
        ...
        for (const roleId of playerRoles) {
            ...
            if (role) {
                role.members = role.members.filter((m: any) => m != playerData.id);
 ...
 */
export function playerChangeOwner(player: any, member: any, countryData: any) {
 ...
 * @returns 
 */
export function changeOwnerScriptEvent(member: any) {
    const memberData = GetAndParsePropertyData(`player_${member.id}`);
 ...
 * @param {Number} countryId 
 */
export function playerCountryInvite(receivePlayer: any, sendPlayer: any) {
    try {
        ...
        receivePlayerData.invite = receivePlayerData.invite.filter((v: any) => v != countryId).concat(Number(countryId));
 ...
 * @param {number} countryId 
 */
export function sendApplicationForPeace(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
     ...
     */
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.applicationPeaceRequestReceive = countryData.applicationPeaceRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
    playerCountryData.applicationPeaceRequestSend = playerCountryData.applicationPeaceRequestSend.filter((r: any) => r != Number(countryId)).concat(Number(countryId));
 ...
 * @param {number} countryId 
 */
export function sendAllianceRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.allianceRequestReceive = countryData.allianceRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
    playerCountryData.allianceRequestSend = playerCountryData.allianceRequestSend.filter((r: any) => r != Number(countryId)).concat(Number(countryId));
    StringifyAndSavePropertyData(`country_${playerData.country}`, playerCountryData);
    StringifyAndSavePropertyData(`country_${countryId}`, countryData);
 ...
 * @param {number} countryId 
*/
export function cancelSendApplicationForPeace(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.applicationPeaceRequestReceive = countryData.applicationPeaceRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
 ...
 * @param {number} countryId 
 */
export function cancelAllianceRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.allianceRequestReceive = countryData.allianceRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
 ...
 * @param {number} countryId 
 */
export function acceptAlliance(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.allianceRequestReceive = countryData.allianceRequestReceive.filter((r: any) => r != playerData.country);
    playerCountryData.allianceRequestSend = playerCountryData.allianceRequestSend.filter((r: any) => r != Number(countryId));
    playerCountryData.allianceRequestReceive = playerCountryData.allianceRequestReceive.filter((r: any) => r != Number(countryId));
 ...
 * @param {number} countryId 
 */
export function denyAllianceRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.allianceRequestSend = countryData.allianceRequestSend.filter((r: any) => r != playerData.country);
    playerCountryData.allianceRequestReceive = playerCountryData.allianceRequestReceive.filter((r: any) => r != Number(countryId));
    StringifyAndSavePropertyData(`country_${playerData.country}`, playerCountryData);
    StringifyAndSavePropertyData(`country_${countryId}`, countryData);
 ...
 * @param {number} countryId 
 */
export function denyApplicationRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.applicationPeaceRequestSend = countryData.applicationPeaceRequestSend.filter((r: any) => r != playerData.country);
    playerCountryData.applicationPeaceRequestReceive = playerCountryData.applicationPeaceRequestReceive.filter((r: any) => r != Number(countryId));
    StringifyAndSavePropertyData(`country_${playerData.country}`, playerCountryData);
    StringifyAndSavePropertyData(`country_${countryId}`, countryData);
 ...
 * @param {number} countryId 
 */
export function acceptApplicationRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.applicationPeaceRequestReceive = countryData.applicationPeaceRequestReceive.filter((r: any) => r != playerData.country);
    playerCountryData.applicationPeaceRequestSend = playerCountryData.applicationPeaceRequestSend.filter((r: any) => r != Number(countryId));
    countryData.applicationPeaceRequestSend = countryData.applicationPeaceRequestSend.filter((r: any) => r != playerData.country);
    playerCountryData.applicationPeaceRequestReceive = playerCountryData.applicationPeaceRequestReceive.filter((r: any) => r != Number(countryId));
    countryData.hostility = countryData.hostility.filter((h: any) => h != playerData.country);
    playerCountryData.hostility = playerCountryData.hostility.filter((h: any) => h != Number(countryId));
    countryData.warNowCountries = countryData.warNowCountries.filter((w: any) => w != playerData.country);
    playerCountryData.warNowCountries = playerCountryData.warNowCountries.filter((w: any) => w != Number(countryId));
    StringifyAndSavePropertyData(`country_${playerData.country}`, playerCountryData);
    StringifyAndSavePropertyData(`country_${countryId}`, countryData);
 ...
 * @param {number} countryId 
 */
export function AddHostilityByPlayer(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.allianceRequestReceive = countryData.allianceRequestReceive.filter((r: any) => r != playerData.country);
    ...
    playerCountryData.allianceRequestSend = playerCountryData.allianceRequestSend.filter((r: any) => r != Number(countryId));
    playerCountryData.allianceRequestReceive = playerCountryData.allianceRequestReceive.filter((r: any) => r != Number(countryId));
    countryData.alliance = countryData.alliance.filter((r: any) => r != playerData.country);
    playerCountryData.alliance = playerCountryData.alliance.filter((r: any) => r != Number(countryId));

    countryData.friendlyRequestReceive = countryData.friendlyRequestReceive.filter((r: any) => r != playerData.country);
    countryData.friendlyRequestSend = countryData.friendlyRequestSend.filter((r: any) => r != playerData.country);
    playerCountryData.friendlyRequestSend = playerCountryData.friendlyRequestSend.filter((r: any) => r != Number(countryId));
    playerCountryData.friendlyRequestReceive = playerCountryData.friendlyRequestReceive.filter((r: any) => r != Number(countryId));
    countryData.friendly = countryData.friendly.filter((r: any) => r != playerData.country);
    playerCountryData.friendly = playerCountryData.friendly.filter((r: any) => r != Number(countryId));
 ...
 * @param {number} countryId 
 */
export function sendFriendlyRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.friendlyRequestReceive = countryData.friendlyRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
    playerCountryData.friendlyRequestSend = playerCountryData.friendlyRequestSend.filter((r: any) => r != Number(countryId)).concat(Number(countryId));
    StringifyAndSavePropertyData(`country_${playerData.country}`, playerCountryData);
    StringifyAndSavePropertyData(`country_${countryId}`, countryData);
 ...
 * @param {number} countryId 
 */
export function cancelFriendlyRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.friendlyRequestReceive = countryData.friendlyRequestReceive.filter((r: any) => r != playerData.country).concat(playerData.country);
 ...
 * @param {number} countryId 
 */
export function acceptFriendly(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.friendlyRequestReceive = countryData.friendlyRequestReceive.filter((r: any) => r != playerData.country);
 ...
 * @param {number} countryId 
 */
export function denyFriendlyRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
    countryData.friendlyRequestSend = countryData.friendlyRequestSend.filter((r: any) => r != playerData.country);
 ...
 * @returns 
 */
export function createPlotGroup(player: any, country: any, name: any, type = "public") {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
 ...
 * @returns 
 */
export function createPlot(player: any, name: any, chunkId: any, type = "public") {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) {
        player.sendMessage({ translate: `cannnot.use.nojoin.country` });
        return;
 ...
 * @returns 
 */
export function createPlotToGroup(player: any, group: any, chunkId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) {
        player.sendMessage({ translate: `cannnot.use.nojoin.country` });
        return;
 ...
 * @param {Player=} player
 */
export function MergeCountry(fromId: any, toId: any, player: any) {
 ...
 * 他国との関係解除
 */
function cleanupRelations(countryDB: any, fromId: any) {
        ...
        try {
            ...
            for (const f of [
                ...
                if (Array.isArray(c[f])) {
                    ...
                    c[f] = c[f].filter((id: any) => String(id) !== String(fromId));
                    if (before !== c[f].length) changed = true;
                }
            }

            if (cleanupFederationReference(c, fromId)) changed = true;
            if (changed) countryDB.set(key, JSON.stringify(c));
        } catch { }
    }
}
 ...
 * 関係解除ユーティリティ
 */
function cleanupRelationsDelete(countryDB: any, targetId: any, fields: any) {
    for (const key of countryDB.idList) {
        const raw = countryDB.get(key);
        if (!raw) continue;
        ...
        try {
            ...
            for (const f of fields) {
                if (Array.isArray(c[f])) {
                    const before = c[f].length;
                    c[f] = c[f].filter((id: any) => String(id) !== String(targetId));
                    if (before !== c[f].length) changed = true;
                }
            }

            if (cleanupFederationReference(c, targetId)) changed = true;
...
function cleanupFederationReference(countryData: any, targetId: any) {
    if (!countryData?.federation || Array.isArray(countryData.federation) || typeof countryData.federation !== "object") return false;
    let changed = false;
    const federation = countryData.federation;

    if (Array.isArray(federation.members)) {
        const before = federation.members.length;
        federation.members = federation.members.filter((id: any) => String(id) !== String(targetId));
        if (before !== federation.members.length) changed = true;
    } else {
        federation.members = [];
        changed = true;
    }

    if (Array.isArray(federation.requests)) {
        const before = federation.requests.length;
        federation.requests = federation.requests.filter((id: any) => String(id) !== String(targetId));
        if (before !== federation.requests.length) changed = true;
    } else {
        federation.requests = [];
        changed = true;
    }

    if (String(federation.capital) === String(targetId)) {
        federation.capital = federation.members[0] ?? "";
        changed = true;
    }

    if (!federation.members.length) {
 ...
 * @param {number} countryId 
 */
export function sendMergeRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
 ...
 * @param {number} countryId 
*/
export function cancelMergeRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
 ...
 * @param {number} countryId 
 */
export function denyMergeRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const playerCountryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const countryData = GetAndParsePropertyData(`country_${countryId}`);
 ...
 * @param {number} countryId 
 */
export function acceptMergeRequest(player: any, countryId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
 ...
 * @returns 
 */
export function addPlotToGroup(player: any, group: any, chunkId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) {
        player.sendMessage({ translate: `cannnot.use.nojoin.country` });
        return;
