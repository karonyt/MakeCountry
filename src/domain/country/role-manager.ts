...
/**@typedef {import("@/types/legacy/permission").CountryPermissionList} CountryPermissionList*/
...
export class RoleManager {
     ...
     *
     * @param {number} roleId
     * @param {{enabled: boolean, taxPer: number, taxInstitutionIsPer: boolean}} value
     */
    setTaxOverride(roleId: any, value: any) {
        const roleData = this.getRole(roleId);
        if (!roleData) return false;
        roleData.taxOverride = {
            enabled: Boolean(value.enabled),
            taxPer: Number(value.taxPer) || 0,
            taxInstitutionIsPer: Boolean(value.taxInstitutionIsPer)
        };
        this.setRole(roleId, roleData);
        return true;
    }
     ...
     */
    moveCountryRole(countryId: any, roleId: any, newIndex: any) {
        const countryDataBase = new DynamicProperties('country');
        const countryRawData = countryDataBase.get(`country_${countryId}`);
        if (!countryRawData) return false;
        const countryData = JSON.parse(countryRawData);
        if (!Array.isArray(countryData.roles)) return false;
        const currentIndex = countryData.roles.indexOf(roleId);
        if (currentIndex < 0) return false;
        const boundedIndex = Math.max(0, Math.min(Number(newIndex), countryData.roles.length - 1));
        const [targetRoleId] = countryData.roles.splice(currentIndex, 1);
        countryData.roles.splice(boundedIndex, 0, targetRoleId);
