...
function formatRemainingMs(remainingMs: number) {
    ...
    if (hours > 0) return { translate: "duration.hours_minutes", with: [`${hours}`, `${minutes}`] };
    if (minutes > 0) return { translate: "duration.minutes_seconds", with: [`${minutes}`, `${seconds}`] };
    return { translate: "duration.seconds", with: [`${seconds}`] };
}
...
export function placeBounty(setter: any, target: any, amount: any) {
    ...
    if (targetCooldownUntil > now) {
        setter.sendMessage({ translate: "bounty.error.target_cooldown", with: { rawtext: [{ text: target.name }, formatRemainingMs(targetCooldownUntil - now)] } });
        return false;
    }
