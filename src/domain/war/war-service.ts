...
import { GetAndParsePropertyData, GetChunkPropertyId, GetPlayerChunkPropertyId, isWithinTimeRange, StringifyAndSavePropertyData } from "@/shared/utils/minecraft.js";
 ...
 * @param {Player} player 
 */
export async function Invade(player: any) {
    let key = 0;
    for (let i = 1; i < 16; i++) {
    ...
    if (areCountriesInSameFederation(playerCountryData, targetCountryData)) {
        player.sendMessage({ rawtext: [{ text: `§a[MakeCountry]\n` }, { translate: `invade.error.federation` }] });
        return;
    };
...
world.afterEvents.entityLoad.subscribe((ev) => {
    ...
    // @ts-ignore TS(2339): Property 'some' does not exist on type 'Map<any, a... Remove this comment to see the full error message
    const isWar = warCountry.some((value: any) => entity.id == value.core);
    if (!isWar) {
        entity.remove();
    };
});
...
world.afterEvents.entityDie.subscribe((ev) => {
    ...
    if (invadeCountryData.territories.includes(chunkData.id)) {
        invadeCountryData.territories = invadeCountryData.territories.filter((id: any) => id != chunkData.id);
    };
...
world.afterEvents.entityDie.subscribe((ev) => {
    ...
    */
    ...
    for (let i = 0; i < 36; i++) {
        ...
        // @ts-ignore TS(2345): Argument of type 'ItemStack | undefined' is not as... Remove this comment to see the full error message
