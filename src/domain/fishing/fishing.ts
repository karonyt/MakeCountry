...
import fishing_config from "@/config/fishing.js";
import { DynamicProperties } from "@/shared/storage/dynamic-properties.js";
import { TimeManager } from "@/domain/time/time.js";

export class FishManager {
    _markDirty: any;
    accessMap: any;
    activeCacheMaxEntries: any;
    calendar: any;
    config: any;
    db: any;
    dirtyPlayerKeys: any;
    dirtyRankKeys: any;
    flushIntervalTicks: any;
    inactiveEvictSeconds: any;
    playerCounts: any;
    playerMaxes: any;
    rankLimit: any;
    serverRanks: any;
    ...
    constructor(options = {}) {
        ...
        // options + tuning
        // @ts-ignore TS(2339): Property 'flushIntervalTicks' does not exist on ty... Remove this comment to see the full error message
        this.flushIntervalTicks = options.flushIntervalTicks ?? 600; // 30s @20tps
        // @ts-ignore TS(2339): Property 'writesPerTick' does not exist on type '{... Remove this comment to see the full error message
        this.writesPerTick = options.writesPerTick ?? 100; // how many writes to process in one flush tick
        // @ts-ignore TS(2339): Property 'activeCacheMaxEntries' does not exist on... Remove this comment to see the full error message
        this.activeCacheMaxEntries = options.activeCacheMaxEntries ?? 500000; // cap for player entries (count+max keys combined)
        // @ts-ignore TS(2339): Property 'inactiveEvictSeconds' does not exist on ... Remove this comment to see the full error message
        this.inactiveEvictSeconds = options.inactiveEvictSeconds ?? 900; // 15 minutes default

        // rank limit
        // @ts-ignore TS(2339): Property 'rankLimit' does not exist on type '{}'.
    ...
    // ---------- helpers ----------
    _playerCountKey(playerId: any, fishId: any) { return `c_${playerId}_${fishId}`; }
    _playerMaxKey(playerId: any, fishId: any) { return `m_${playerId}_${fishId}`; }
    _serverRankKey(fishId: any) { return `server_${fishId}_rank`; }
    _nowSec() { return Math.floor(Date.now() / 1000); }
    _storeSizeInt(sizeFloat: any) { return Math.floor(sizeFloat * 100); }
    _restoreSizeFloat(sizeInt: any) { return (sizeInt / 100); }

    // ---------- preload ranks ----------
    _getAllFishIdsFromConfig() {
        if (Array.isArray(this.config.allFishes)) {
            return this.config.allFishes.map((f: any) => f.typeId);
        }

        const ids = new Set();
        const date = this.calendar.getCalendar();
        (this.config.fishes[date.season][date.period] ?? []).forEach((f: any) => ids.add(f.typeId));
        const bi = this.config.biomes ?? {};
        // @ts-ignore TS(2571): Object is of type 'unknown'.
        Object.values(bi).forEach(b => (b.fishes[date.season][date.period] ?? []).forEach((f: any) => ids.add(f.typeId)));
    ...
    _loadPlayerCountIfMissing(playerId: any, fishId: any) {
    ...
    _loadPlayerMaxIfMissing(playerId: any, fishId: any) {
    ...
    _pickFish(location: any, dimension: any) {
     ...
     * @param {Player} player 
     */
    fishing(location: any, dimension: any, player: any) {
        ...
        const ranking = this._updateServerRankingInMemory(pId, typeId, sizeInt, player.name);
    ...
    _updateServerRankingInMemory(playerId: any, fishId: any, sizeInt: any, playerName: any = undefined) {
        ...
        const existingIdx = top.findIndex((r: any) => r.playerId === playerId);

        if (existingIdx >= 0) {
            if (playerName && top[existingIdx].playerName !== playerName) {
                top[existingIdx].playerName = playerName;
            ...
            if (top.length < this.rankLimit || sizeInt > top[top.length - 1].size) {
                top.push({ playerId, playerName, size: sizeInt });
                top.sort((a: any, b: any) => b.size - a.size);
                if (top.length > this.rankLimit) top.length = this.rankLimit;
                this._enqueueDirtyRank(fishId);
            }
        }
        ...
        return { top: (rankObj.top.map((r: any) => ({
            playerId: r.playerId,
    ...
    mergePlayerFishRecord(playerId: any, fishId: any, record: any) {
        if (!playerId || !fishId || !record) return this.getPlayerFishRecord(playerId, fishId);
        ...
        const currentCount = this.playerCounts.get(countKey) ?? 0;
        const nextCount = Math.max(currentCount, Number(record.count) || 0);
        if (nextCount > currentCount) {
            this.playerCounts.set(countKey, nextCount);
            this._enqueueDirtyKey(countKey);
        } else {
            this._touchKey(countKey);
        }

        const currentMax = this.playerMaxes.get(maxKey) ?? 0;
        const recordMaxSize = Math.max(Number(record.maxSize) || 0, Number(record.size) || 0);
        const nextMax = Math.max(currentMax, this._storeSizeInt(recordMaxSize));
        if (nextMax > currentMax) {
            this.playerMaxes.set(maxKey, nextMax);
            this._enqueueDirtyKey(maxKey);
        } else {
            this._touchKey(maxKey);
        }
        ...
        if (nextMax > 0) {
            this._updateServerRankingInMemory(playerId, fishId, nextMax, record.playerName);
        }

        if ((this.playerCounts.size + this.playerMaxes.size) > this.activeCacheMaxEntries) {
            this._evictLRU(Math.max(1, Math.floor(this.activeCacheMaxEntries * 0.1)));
        }

        return {
            count: nextCount,
            maxSize: this._restoreSizeFloat(nextMax),
            obtained: nextCount > 0,
        };
    }

    mergePlayerFishRecords(playerId: any, records: any, playerName: any = undefined) {
        if (!records) return;

        const entries = Array.isArray(records)
            ? records
            : Object.entries(records).map(([fishId, record]) => ({
                ...(typeof record === "object" && record ? record : {}),
                typeId: fishId,
            }));

        for (const record of entries) {
            const fishId = record?.typeId ?? record?.fishId;
            if (!fishId) continue;
            this.mergePlayerFishRecord(playerId, fishId, {
                ...record,
                playerName: record.playerName ?? playerName,
            });
        }
    }

    // ---------- LRU eviction (evict N least recently used primitive keys) ----------
    _evictLRU(evictCount: any) {
    ...
    flushAllNow() {
        ...
        for (const key of this.dirtyPlayerKeys) {
            if (!this.writeQueue.find((i: any) => i.key === key && i.type === 'primitive')) {
                this.writeQueue.push({ key, type: 'primitive' });
            }
        }
        for (const fishId of this.dirtyRankKeys) {
            if (!this.writeQueue.find((i: any) => i.key === fishId && i.type === 'rank')) {
                this.writeQueue.push({ key: fishId, type: 'rank' });
            }
        }
     ...
     * @returns {number}
     */
    setRandomSize(min: any, max: any) {
        const v = Math.random() * (max - min) + min;
        return Math.round(v * 100) / 100;
    }

    getPlayerFishRecord(playerId: any, fishId: any) {
        const countKey = this._playerCountKey(playerId, fishId);
        const maxKey = this._playerMaxKey(playerId, fishId);
    ...
    getPlayerAllFishRecords(player: any) {
        const result = {};
        const pid = player.id;
    ...
    getPlayerObtainedFishRecords(playerId: any) {
        const result = [];

        for (const f of this.config.allFishes) {
            const record = this.getPlayerFishRecord(playerId, f.typeId);
            if (!record.obtained) continue;
    ...
    getServerFishRanking(fishId: any) {
        let obj = this.serverRanks.get(fishId);
        if (!obj) {
            const raw = this.db.get(this._serverRankKey(fishId));
            try {
                obj = raw ? JSON.parse(raw) : { top: [] };
            } catch {
                obj = { top: [] };
            }
            this.serverRanks.set(fishId, obj);
        }

        return {
            top: obj.top.map((r: any) => ({
                playerId: r.playerId,
                playerName: r.playerName,
                size: this._restoreSizeFloat(r.size)
            }))
...
let sharedFishManager: FishManager | undefined;

export function getFishManager() {
    sharedFishManager ??= new FishManager();
    return sharedFishManager;
}
