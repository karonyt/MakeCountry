...
import { callCountryListForm } from "@/features/forms/form-registry.js";

function friendlyListExecuter(origin: any, args: any) {
    ...
    // @ts-ignore TS(2345): Argument of type '"fl"' is not assignable to param... Remove this comment to see the full error message
...
system.beforeEvents.startup.subscribe((event) => {
    event.customCommandRegistry.registerCommand(
        ...
        ((origin, ...args) => {
            system.runTimeout(() => {
                friendlyListExecuter(origin, args);
            })
        })
    )
