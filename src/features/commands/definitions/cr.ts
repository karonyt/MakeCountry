...
function crExecuter(origin: any, args: any) {
    ...
    if (item) {
        ...
        if (loreArray.includes(`§c§r§d${sender.name}(${sender.id})`)) {
            item.setLore(loreArray.filter((lore: any) => lore != `§c§r§d${sender.name}(${sender.id})`));
            container.setItem(sender.selectedSlotIndex, item);
            return;
        };
        if (loreArray.find((lore: any) => lore.includes(`§c§r§d`))) {
