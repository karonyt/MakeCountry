...
import { GetAndParsePropertyData, GetPlayerChunkPropertyId } from "@/shared/utils/minecraft.js";
import { getFederationMemberIds, normalizeFederationData } from "@/domain/country/relationships/federation.js";

function mapExecuter(origin: any, args: any) {
    ...
    const federationData = normalizeFederationData(playerCountryData);
    const federation = federationData.settings?.showOnMap
        ? getFederationMemberIds(playerCountryData).filter((id: any) => Number(id) !== Number(playerData.country))
        : [];
    ...
    for (let i = 10; i > -11; i--) {
        ...
        for (let j = -10; j < 11; j++) {
            ...
            if (chunkData?.countryId) {
                ...
                if (federation.includes(chunkData?.countryId)) {
                    colorCode = "9";
                };
