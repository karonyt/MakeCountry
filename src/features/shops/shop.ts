...
import { ModalForm } from "@/shared/ui/form-class.js";
...
export function ShopCommonsMenu(player: any, page = 0) {
    const form = new ChestFormData("large").setTitle(`shop.title.admin`);
    let lv = 0;
    ...
    // @ts-ignore TS(2532): Object is possibly 'undefined'.
    form.setButton(0, { name: "shop.button.close", iconPath: itemIdToPath["minecraft:barrier"], lore: ["shop.button.close.lore"], editedName: true });
    // @ts-ignore TS(2532): Object is possibly 'undefined'.
    if ((page + 1) * 45 < commonsAll.length) form.setButton(5, { name: "shop.button.next", iconPath: "textures/ui/arrow_right", lore: ["shop.button.next.lore"], editedName: true });
    // @ts-ignore TS(2532): Object is possibly 'undefined'.
    if (0 < page) form.setButton(3, { name: "shop.button.previous", iconPath: "textures/ui/arrow_left", lore: ["shop.button.previous.lore"], editedName: true });
 ...
 * @param {Player} player 
 */
export function ShopCommonsMenuCategory(player: any, categoryCommons: any, page = 0, keyword = ``, type = 0) {
    const form = new ChestFormData("large").setTitle(`shop.title.admin`);
    /**
 ...
 */
export function ShopSelectCommonForm(player: any, common: any, categoryCommons: any, page: any) {
