...
import { playerChatMuteMenuForm } from "@/features/forms/default/menu/setting/chat_mute.js";
...
export function playerSettingDefaultForm(player: any) {
    ...
    form.body({ translate: "settings.menu.body" });
    form.button({ translate: "settings.button.basic" });
    form.button({ translate: "settings.button.chat_mute" });
...
function playerBasicSettingDefaultForm(player: any) {
    ...
    form.title({ translate: 'form.mainmenu.button.setting' });
    form.dropdown({ translate: 'settings.ui_type' }, ['default', 'kingdoms', 'towny'], { defaultValueIndex: index });
    ...
    let settings = [
        ...
        {
            translate: 'mute.federationchat',
            dypKey: 'isMuteFederationChat',
            lv: 0
        },
        {
