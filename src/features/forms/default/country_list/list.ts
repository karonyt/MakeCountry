...
import { showCountryInfoDefaultForm } from "@/features/forms/default/country_list/show.js";
...
function warnCountryListData(message: any, context: any = {}) {
    const details = Object.entries(context)
        .map(([key, value]) => `${key}=${String(value)}`)
        .join(" | ");
    console.warn(`[MakeCountry CountryList] ${message}${details ? ` | ${details}` : ""}`);
}
...
export function countryListDefaultForm(
    ...
    if (!type) {
        ...
        countryIds = getFederationMemberIds(pcd).map((id: any) => `country_${id}`) ?? [];
    } else if (type === 'fl') {
    ...
    for (const id of countryIds) {
        ...
        const property = describeDynamicPropertyEntry("country", id);
        if (!raw) {
            warnCountryListData("Missing country dynamic property while building list", {
                key: id,
                property,
                type: type ?? "all",
            });
        ...
        try {
            ...
            if (countryData?.id === undefined || countryData?.id === null) {
                warnCountryListData("Country data has no id field while building list", {
                    key: id,
                    property,
            ...
            warnCountryListData("Failed to parse country dynamic property while building list", {
                key: id,
                property,
                error,
            });
    ...
    const maxPage = Math.max(0, Math.ceil(filteredCountries.length / PAGE_SIZE) - 1);
