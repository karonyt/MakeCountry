...
import { StewRecipes } from "@/content/cooking/stew-recipe.js";
...
export function openRecipeSelect(player: any, page = 0, categoryId = "all", searchQuery = "") {
    ...
    const lv = new JobLevel(player, "chef").getLevel();
    const recipes = StewRecipes as Record<string, any>;
...
function openPot(player: any) {
    ...
    form.setButton(18, {
        ...
        lore: [
            ...
            { rawtext: [{ text: '§5' }, { translate: "cooking.lore.quality", with: [`${capCookingRankByChefLevel(player, calcRank(pot, recipe)).rank}`] }] }
        ],
        editedName: true
    });
...
function finish(player: any) {
    ...
    const { rank, mul } = capCookingRankByChefLevel(player, calcRank(pot, recipe));
