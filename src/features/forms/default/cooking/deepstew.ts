...
import { DeepStewRecipes } from "@/content/cooking/deepstew-recipe.js";
...
function isBurnDanger(pot: any, recipe: any) {
    return (
...
function heatVariance(player: any, recipe: any) {
    const lv = new JobLevel(player, "chef").getLevel();
...
function seasoningSuccessRate(pot: any, recipe: any) {
    let rate = 1;
...
const DEEP_STEW_RANK_MULTIPLIER = {
    "KARON": 15,
    "IMPOSSIBLE": 8,
    "EXTRA": 5,
    "LEGENDARY": 3.0,
...
const DeepPotActions = [
    /* ===== 基本操作 ===== */
    {
        ...
        unlockLv: 1,
        cost: 1,
        run(pot: any, recipe: any, ctx: any, player: any) {
...
/* ===============================
   UI
================================ */
export function openDeepStewRecipeSelect(player: any, page = 0, categoryId = "all", searchQuery = "") {
    const form = new ChestFormData("large")
        .setTitle({ rawtext: [{ translate: "cooking.title.deep_pot" }] });

    const lv = new JobLevel(player, "chef").getLevel();
    const recipes = DeepStewRecipes as Record<string, any>;
    ...
    form.show(player).then(r => {
        ...
        if (r.selection === 45) {
            openDeepStewRecipeSelect(player, safePage - 1, selectedCategory.id, cleanSearchQuery);
            return;
        }
        if (r.selection === 53) {
            openDeepStewRecipeSelect(player, safePage + 1, selectedCategory.id, cleanSearchQuery);
        ...
        if (r.selection === 47) {
            openDeepStewRecipeSelect(player, 0, adjacentRecipeCategory(recipes, selectedCategory.id, -1).id, cleanSearchQuery);
            return;
        }
        if (r.selection === 51) {
            openDeepStewRecipeSelect(player, 0, adjacentRecipeCategory(recipes, selectedCategory.id, 1).id, cleanSearchQuery);
        ...
        if (r.selection === 48) {
            openDeepStewRecipeSearch(player, selectedCategory.id, cleanSearchQuery);
            return;
        }
        if (r.selection === 50 && searchActive) {
            openDeepStewRecipeSelect(player, 0, selectedCategory.id);
            return;
        }
        ...
        if (!recipe || lv < recipe.requiredLevel) {
            openDeepStewRecipeSelect(player, safePage, selectedCategory.id, cleanSearchQuery);
            return;
        }
...
function openDeepStewRecipeSearch(player: any, categoryId: string, searchQuery = "") {
    ...
    form.show(player).then(r => {
        if (r.canceled) return;
        openDeepStewRecipeSelect(player, 0, categoryId, String(r.formValues?.[0] ?? ""));
    });
}
...
function startCooking(player: any, recipeId: any, quantity: any) {
    // @ts-ignore TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    const recipe = DeepStewRecipes[recipeId];
...
function openDeepPot(player: any) {
    ...
    form.setButton(18, {
        ...
        lore: [
            ...
            { rawtext: [{ text: '§5' }, { translate: "cooking.lore.quality", with: [`${capCookingRankByChefLevel(player, calcRank(pot, recipe), DEEP_STEW_RANK_MULTIPLIER).rank}`] }] }
        ],
        editedName: true
    });
...
function calcHeatScore(heat: any, needHeat: any, maxScore = 5) {
...
function calcEfficiencyBonus(pot: any, recipe: any) {
...
function calcRank(pot: any, recipe: any) {
    let score = pot.score;
...
function finish(player: any) {
    ...
    const { rank, mul } = capCookingRankByChefLevel(player, calcRank(pot, recipe), DEEP_STEW_RANK_MULTIPLIER);
