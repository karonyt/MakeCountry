...
import { ActionForm, ModalForm } from "@/shared/ui/form-class.js";
const ActionFormData = ActionForm;
const ModalFormData = ModalForm;
import config from "@/config/server.js";
import { createPlotGroup } from "@/domain/country/land-service.js";
import { CheckPermission, GetAndParsePropertyData, HasPermission, isDecimalNumberZeroOK, StringifyAndSavePropertyData } from "@/shared/utils/minecraft.js";
 ...
 * @param {Player} player 
 */
export function settingCountryPlotGroupForm(player: any) {
 ...
 * @param {Player} player  
 */
export function CreatePlotGroupForm(player: any) {
 ...
 */
export function plotGroupEditMainFormPlotAdmin(player: any, plotGroupId: any) {
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 6: {
                ...
                // @ts-ignore TS(2554): Expected 2-3 arguments, but got 1.
 ...
 */
export function plotGroupEditMainFormPlotOwner(player: any, plotGroupId: any) {
 ...
 */
function plotGroupOwnerShowForm(player: any, plotGroupId: any, plotAdmin = false) {
    let plot = GetAndParsePropertyData(`plotgroup_${plotGroupId}`);
    if (!plot) {
        return;
    };
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 2: {
                ...
                // @ts-ignore TS(2304): Cannot find name 'chunkId'.
 ...
 * @param {Player} player 
 */
function plotGroupEditRolesListForm(player: any, plotGroupId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
    ...
    plot.roles = plot.roles.filter((d: any) => aliveRoles.includes(d.id));
 ...
 */
export function roleAddPlotGroupForm(player: any, plotGroupId: any, plotAdmin: any, search = false, keyword = ``) {
    const form = new ActionFormData();
    ...
    if (search) {
        // @ts-ignore TS(7006): Parameter 'r' implicitly has an 'any' type.
    ...
    let aliveRoles: any = [];
    for (const r of roles) {
        if (plot.roles.find((d: any) => d?.id == r.id)) continue;
        aliveRoles.push(r)
 ...
 * @param {boolean} plotAdmin 
 */
function plotGroupRoleSelectedShowForm(player: any, targetData: any, plotGroupId: any, plotAdmin = false) {
    const form = new ActionFormData();
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 2: {
                ...
                if (!plot?.roles) plot.roles = [];
                plot.roles.splice(plot.roles.indexOf((d: any) => d.id == targetData.id), 1);
                StringifyAndSavePropertyData(`plotgroup_${plotGroupId}`, plot);
                plotGroupEditRolesListForm(player, plotGroupId, plotAdmin);
                break;
            };
 ...
 * @param {Player} player 
 */
function plotGroupRolePermissionsEditForm(player: any, plotGroupId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    ...
    if (!plot?.roles) plot.roles = [];
    let target = plot.roles.find((d: any) => d.id == targetData?.id) ?? { id: targetData?.id, permissions: [] };
    // @ts-ignore TS(2552): Cannot find name 'landPermissions'. Did you mean '... Remove this comment to see the full error message
    ...
    form.show(player).then((rs: any) => {
        ...
        const index = plot?.roles.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotGroupEditCountriesListForm(player: any, plotGroupId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
    ...
    plot.countries = plot.countries.filter((d: any) => aliveCountries.includes(d.id));
    StringifyAndSavePropertyData(`plotgroup_${plotGroupId}`, plot);
 ...
 * @param {string} keyword 
 */
export function countryAddPlotGroupForm(player: any, plotGroupId: any, plotAdmin: any, search = false, keyword = ``) {
    const form = new ActionFormData();
    ...
    if (search) {
        // @ts-ignore TS(7006): Parameter 'c' implicitly has an 'any' type.
 ...
 * @param {boolean} plotAdmin 
 */
function plotGroupCountrySelectedShowForm(player: any, targetData: any, plotGroupId: any, plotAdmin = false) {
    const form = new ActionFormData();
    form.title({ text: `${targetData?.name}` });
    form.button({ translate: `mc.button.back` });
    form.button({ translate: `mc.button.edit.permission` });
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 2: {
                ...
                if (!plot?.countries) plot.countries = [];
                plot.countries = plot.countries.filter((d: any) => d.id != targetData.id);
                StringifyAndSavePropertyData(`plotgroup_${plotGroupId}`, plot);
                plotGroupEditCountriesListForm(player, plotGroupId, plotAdmin);
                break;
            };
 ...
 * @param {Player} player 
 */
function plotGroupCountryPermissionsEditForm(player: any, plotGroupId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
    ...
    if (!plot?.countries) plot.countries = [];
    let target = plot.countries.find((d: any) => d.id == targetData?.id) ?? { id: targetData?.id, permissions: [] };
    // @ts-ignore TS(2552): Cannot find name 'landPermissions'. Did you mean '... Remove this comment to see the full error message
    for (const permission of landPermissions) {
        form.toggle({ translate: `permission.${permission}` }, target?.permissions.includes(permission));
    };
    ...
    form.show(player).then((rs: any) => {
        ...
        target.permissions = newLandPermissions;
        const index = plot?.countries.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotGroupEditPlayersListForm(player: any, plotGroupId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
 ...
 * @param {string} keyword 
 */
export function playerAddPlotGroupForm(player: any, plotGroupId: any, plotAdmin: any, serch = false, keyword = ``) {
    const form = new ActionFormData();
    ...
    let showPlayers: any = [];
    for (const p of players) {
        if (plot.players.find((d: any) => d?.id == p.id)) continue;
 ...
 * @param {boolean} plotAdmin 
 */
function plotGroupPlayerSelectedShowForm(player: any, targetData: any, plotGroupId: any, plotAdmin = false) {
    const form = new ActionFormData();
    form.title({ text: `${targetData?.name}` });
    form.button({ translate: `mc.button.back` });
    form.button({ translate: `mc.button.edit.permission` });
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 2: {
                ...
                if (!plot?.players) plot.players = [];
                plot.players.splice(plot.players.indexOf((d: any) => d.id == targetData.id), 1);
                StringifyAndSavePropertyData(`plotgroup_${plotGroupId}`, plot);
                plotGroupEditPlayersListForm(player, plotGroupId, plotAdmin);
                break;
            };
 ...
 * @param {Player} player 
 */
function plotGroupPlayerPermissionsEditForm(player: any, plotGroupId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
    ...
    if (!plot?.players) plot.players = [];
    let target = plot.players.find((d: any) => d.id == targetData?.id) ?? { id: targetData?.id, permissions: [] };
    // @ts-ignore TS(2552): Cannot find name 'landPermissions'. Did you mean '... Remove this comment to see the full error message
    for (const permission of landPermissions) {
        form.toggle({ translate: `permission.${permission}` }, target?.permissions.includes(permission));
    };
    ...
    form.show(player).then((rs: any) => {
        ...
        target.permissions = newLandPermissions;
        const index = plot?.players.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotGroupEditPermissionsForm(player: any, plotGroupId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
 ...
 * @param {Player} player 
 */
function plotGroupEditSettingFormPlotAdmin(player: any, plotGroupId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
 ...
 * @param {Player} player 
 */
function plotGroupEditSettingFormPlotOwner(player: any, plotGroupId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.settings` }] });
