...
import { CheckPermissionFromLocation, GetAndParsePropertyData, GetPlayerChunkPropertyId, isDecimalNumberZeroOK, StringifyAndSavePropertyData } from "@/shared/utils/minecraft.js";
import { Player, system, world } from "@minecraft/server";
import { addPlotToGroup, createPlot, createPlotToGroup } from "@/domain/country/land-service.js";
import config from "@/config/server.js";
import { plotGroupEditMainFormPlotOwner } from "@/features/forms/legacy-country-forms.js";
 ...
 * @param {Player} player  
 */
export function plotMainForm(player: any) {
    ...
    body.push(
        ...
        // @ts-ignore TS(2345): Argument of type '{ translate: string; with: strin... Remove this comment to see the full error message
 ...
 */
export function plotBuyForm(player: any, chunkId: any) {
    const chunkData = GetAndParsePropertyData(`${chunkId}`);
    let plot = chunkData?.plot ?? {
        group: undefined,
        is_selling: false,
 ...
 * @param {string} chunkId 
 */
export function plotGroupBuyForm(player: any, plotgroupId: any) {
 ...
 * @param {Player} player  
 */
export function CreatePlotForm(player: any, chunkId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
 ...
 */
export function plotEditMainFormPlotAdmin(player: any, plot: any, chunkId: any) {
    const form = new ActionFormData();
 ...
 * @param {{is_selling: boolean,group: string|undefined,country: number|undefined,name: string|undefined,owner: string|undefined,permissions: [string],roles: [{id: number,permissions: [string]}],countries: [{id: number,permissions: [string]}],players: [{id: string,permissions: [string]}],type: "public"|"private"|"embassy",price: number|0, } | undefined} plot
 */
export function plotEditMainFormPlotOwner(player: any, plot: any, chunkId: any) {
    const form = new ActionFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.title` }] });
    form.button({ translate: `plot.edit.menu.button.settings` });
    form.button({ translate: `plot.edit.menu.button.permissions` });
 ...
 */
function plotOwnerShowForm(player: any, chunkId: any, plotAdmin = false) {
 ...
 * @param {Player} player 
 */
function plotEditRolesListForm(player: any, chunkId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
 ...
 */
export function roleAddPlotForm(player: any, chunkId: any, plotAdmin: any, search = false, keyword = ``) {
    const form = new ActionFormData();
 ...
 * @param {boolean} plotAdmin 
 */
function roleSelectedShowForm(player: any, targetData: any, chunkId: any, plotAdmin = false) {
    const form = new ActionFormData();
 ...
 * @param {Player} player 
 */
function rolePermissionsEditForm(player: any, chunkId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    ...
    form.show(player).then((rs: any) => {
        ...
        const index = chunkData?.plot?.roles.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotEditCountriesListForm(player: any, chunkId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
 ...
 * @param {string} keyword 
 */
export function countryAddPlotForm(player: any, chunkId: any, plotAdmin: any, search = false, keyword = ``) {
    const form = new ActionFormData();
 ...
 * @param {boolean} plotAdmin 
 */
function countrySelectedShowForm(player: any, targetData: any, chunkId: any, plotAdmin = false) {
    const form = new ActionFormData();
    form.title({ text: `${targetData?.name}` });
    form.button({ translate: `mc.button.back` });
    form.button({ translate: `mc.button.edit.permission` });
    ...
    form.show(player).then((rs: any) => {
        ...
        switch (rs.selection) {
            ...
            case 2: {
                ...
                if (!plot?.countries) plot.countries = [];
                // @ts-ignore TS(2304): Cannot find name 'aliveCountries'.
                plot.countries = plot.countries.filter((d: any) => aliveCountries.includes(d.id));
                chunkData.plot = plot;
                StringifyAndSavePropertyData(`${chunkId}`, chunkData);
                plotEditCountriesListForm(player, chunkId, plotAdmin);
 ...
 * @param {Player} player 
 */
function countryPermissionsEditForm(player: any, chunkId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
    ...
    form.show(player).then((rs: any) => {
        ...
        target.permissions = newLandPermissions;
        const index = chunkData?.plot?.countries.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotEditPlayersListForm(player: any, chunkId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ActionFormData();
 ...
 * @param {string} keyword 
 */
export function playerAddPlotForm(player: any, chunkId: any, plotAdmin: any, serch = false, keyword = ``) {
    const form = new ActionFormData();
 ...
 * @param {boolean} plotAdmin 
 */
function playerSelectedShowForm(player: any, targetData: any, chunkId: any, plotAdmin = false) {
    const form = new ActionFormData();
    form.title({ text: `${targetData?.name}` });
    form.button({ translate: `mc.button.back` });
    form.button({ translate: `mc.button.edit.permission` });
 ...
 * @param {Player} player 
 */
function playerPermissionsEditForm(player: any, chunkId: any, targetData: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
    ...
    form.show(player).then((rs: any) => {
        ...
        target.permissions = newLandPermissions;
        const index = chunkData?.plot?.players.findIndex((d: any) => d.id == target.id);
        if (index != -1) {
 ...
 * @param {Player} player 
 */
function plotEditPermissionsForm(player: any, chunkId: any, plotAdmin = false) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.permissions` }] });
 ...
 * @param {Player} player 
 */
function plotEditSettingFormPlotAdmin(player: any, chunkId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const countryData = GetAndParsePropertyData(`country_${playerData?.country}`);
    const form = new ModalFormData();
 ...
 * @param {Player} player 
 */
function plotEditSettingFormPlotOwner(player: any, chunkId: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    if (!playerData?.country) return;
    const form = new ModalFormData();
    form.title({ rawtext: [{ translate: `plot.edit.menu.button.settings` }] });
