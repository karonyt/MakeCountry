...
import { CheckPermission, GetAndParsePropertyData, GetPlayerChunkPropertyId, isDecimalNumber, isDecimalNumberZeroOK, StringifyAndSavePropertyData } from "@/shared/utils/minecraft.js";
 ...
 * @param {Player} player 
 */
export function kingdomsSettingForm(player: any) {
...
function colorSetFormMain(player: any) {
    const form = new ChestFormData();
...
function colorInputForm(player: any) {
...
function taxForm(player: any) {
    ...
    // @ts-ignore TS(2559): Type 'string' has no properties in common with typ... Remove this comment to see the full error message
 ...
 * @param {Player} player 
 */
export function resourcePointConverter(player: any) {
    const playerData = GetAndParsePropertyData('player_' + player.id);
    const countryData = GetAndParsePropertyData('country_' + playerData.country);
 ...
 * @param {Player} player 
 */
function withDrawRPForm(player: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const countryData = GetAndParsePropertyData(`country_${playerData.country}`);
    const form = new ModalFormData();
 ...
 * @param {Player} player 
 */
function depositRPForm(player: any) {
    const playerData = GetAndParsePropertyData(`player_${player.id}`);
    const form = new ModalFormData();
