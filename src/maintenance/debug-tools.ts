...
import { GetAndParsePropertyData, GetPlayerChunkPropertyId, StringifyAndSavePropertyData } from "@/shared/utils/minecraft.js";
import { changeOwnerScriptEvent, DeleteCountry, playerCountryJoin } from "@/domain/country/land-service.js";
...
import { updateRanking } from "@/domain/ranking/ranking-service.js";
import { fixCountryData, resetPlotData, resetRoleData } from "@/maintenance/fix-data.js";
import { DynamicProperties } from "@/shared/storage/dynamic-properties.js";
import { addCompeByItemStack } from "@/domain/player/compensation.js";
...
const needs = {
    ...
    // @ts-ignore TS(1117): An object literal cannot have multiple properties ... Remove this comment to see the full error message
    7: {
        item: [
...
system.afterEvents.scriptEventReceive.subscribe((ev) => {
    ...
    switch (ev.id) {
        ...
        case `karo:countrydata`: {
            ...
            const countryData = countryDataBase.get(`country_${message}`);
            console.log(countryData)
            sourceEntity.sendMessage(`${countryData}`);
            break;
        };
        ...
        case 'karo:mergechunk': {
            ...
            chunkDataBase.idList.map((id: any) => {
                /**
        ...
        case 'karo:getchunk': {
            const chunkDataBase = new DynamicProperties('chunk');
            const chunkData = chunkDataBase.get(GetPlayerChunkPropertyId(ev.sourceEntity));
            ev.sourceEntity.sendMessage(`${chunkData}`);
        }
                ...
                if (playerData?.country) {
                    if (playerData.country > 0) {
                        ...
                        if (rawCountryData) {
                            ...
                            if (peopleRoleRawData) {
                                const peopleRoleData = JSON.parse(peopleRoleRawData);
                                peopleRoleData.members = peopleRoleData.members.filter((m: any) => m != playerData.id);
 ...
 * @param {Player} player 
 */
function itemTestForm(player: any) {
